...
#[repr(C)]
#[derive(Debug, Clone)]
pub struct ShapeAreaInstance {
    pub shape: Shape,
    pub color: Color,
}

impl<AreaID> From<ShapeArea<AreaID>> for ShapeAreaInstance {
    fn from(value: ShapeArea<AreaID>) -> Self {
        Self {
            shape: value.shape,
            color: value.color,
        }
...
pub struct MenuRenderState {
    ...
    pub shape_area_cache: Vec<ShapeArea<RootAreaID>>,
    pub shape_area_buffer: Option<wgpu::Buffer>,
...
impl MenuRenderState {
    pub fn new(device: &wgpu::Device, queue: &wgpu::Queue, surface_format: wgpu::TextureFormat) -> Self {
        ...
        let render_pipeline = device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
            ...
            vertex: wgpu::VertexState {
                ...
                buffers: &[
                    ...
                    wgpu::VertexBufferLayout {
                        array_stride: std::mem::size_of::<ShapeAreaInstance>() as wgpu::BufferAddress,
                        ...
                        attributes: &[
                            ...
                            wgpu::VertexAttribute {
                                ...
                                offset: std::mem::size_of::<Rectangle>() as wgpu::BufferAddress,
                                shader_location: 3,
                            },
                        ],
                    },
        ...
        Self {
            ...
            text_renderer,
            rect_vertex_buffer,
            shape_area_cache: vec![],
            shape_area_buffer: None,
    ...
    pub fn render(&self, render_pass: &mut wgpu::RenderPass) -> Result<()> {
        let instance_buffer = match &self.shape_area_buffer { Some(b) => b, None => return Ok(()) };
        
        render_pass.set_pipeline(&self.render_pipeline);
        render_pass.set_vertex_buffer(0, self.rect_vertex_buffer.slice(..));
        render_pass.set_vertex_buffer(1, instance_buffer.slice(..));
        render_pass.set_bind_group(0, &self.bind_group, &[]);
        render_pass.draw(0..RECT_VERTICES.len() as u32, 0..self.shape_area_cache.len() as u32);
    ...
    pub fn find_box_at(&self, position: LogicalPosition<f32>) -> Option<RootAreaID> {
        for shape_area in self.shape_area_cache.iter().rev() {
            if shape_area.shape.contains_point(Vector([position.x, position.y])) {
                if let RootAreaID::Pass = shape_area.id { continue }
                return Some(shape_area.id)
            }
        }
        return None
    }
